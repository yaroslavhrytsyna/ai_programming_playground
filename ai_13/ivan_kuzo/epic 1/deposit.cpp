...
int main(){
    ...
    double result=pow(1+percentage/period,period*years);
    result*=sum;
    double result2=result-sum;
    printf("Hello %s\n",name);
    printf("The compound interest for a principal of %.lf at a rate of %.lf compounded %.i times a year for %.i years is: %.lf\n",sum,percentage,period,years,result2);
